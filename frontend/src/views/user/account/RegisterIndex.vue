...
async function handleRegister(){
  ...
  }else if (passwordConfirmed.value.trim() !== password.value){
  ...
  <div class="ml-[33.33%] mt-30" >
    <form @submit.prevent="handleRegister" class="fieldset bg-base-200 border-base-300 rounded-box w-xs border p-4">
      ...
      <div class="flex justify-end">
        <RouterLink :to="{name:'user-account-login-index'}" class="btn btn-sm btn-ghost text-gary-500">登录</RouterLink>
      </div>
    </form>
  </div>
</template>
